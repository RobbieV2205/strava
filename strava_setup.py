"""
Strava Setup — eenmalig uitvoeren
----------------------------------
Voert de volledige initiële setup uit:
  1. Maakt de MySQL database en applicatiegebruiker aan (via root)
  2. Maakt de `runs` tabel aan
  3. Authoriseert via Strava OAuth2
  4. Haalt alle historische activiteiten op en schrijft ze naar MySQL

Na dit script hoef je alleen nog strava_sync.py te gebruiken voor dagelijkse syncs.

Gebruik:
    1. Vul .env in (zie .env.example)
    2. python strava_setup.py
"""
...
def setup_database():
    """Maakt database en applicatiegebruiker aan via root credentials."""
...
def connect_as_user() -> mysql.connector.MySQLConnection:
...
def fetch_all_runs(token: str) -> list[dict]:
    """Haalt alle runs op van Strava (gepagineerd)."""
    runs, page = [], 1
    ...
    while True:
        ...
        print(
            f"[strava]   pagina {page}: {len(batch)} activiteiten, "
            f"{len(run_batch)} runs (totaal: {len(runs)})"
        )
        page += 1
...
def main():
    ...
    # Stap 2 — Verbinden als applicatiegebruiker en tabel aanmaken
    print(f"[mysql] Verbinden als {MYSQL_USER}...")
    conn = connect_as_user()
    create_table(conn)

    # Stap 3 — Strava autorisatie
    token = get_access_token()

    # Stap 4 — Alle runs ophalen en importeren
    ...
    conn.close()
    print(f"\n[klaar] Setup voltooid. Database: {MYSQL_DATABASE} @ {MYSQL_HOST}")
    print("Je kunt nu strava_sync.py gebruiken voor toekomstige syncs.")
